    ...
    def run(self):
        ...
        while count_of_iteration <= self.count_of_requests:
            ...
            if self.is_theader_stop:
                self.signals.on_iteration.emit(
                    self.theader_id,
                    count_of_iteration,
            ...
            try:
                ...
                if response.status_code in [200, 301, 302]:
                    ...
                    self.signals.on_request.emit(
                        "request", response.status_code, download_size
                    )
                    self.signals.on_log_message.emit(
                        self.theader_id,
                        count_of_iteration,
                        self.cycle_count,
...
class main_gui(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.sb_requests.valueChanged.connect(
        ...
        if os.path.isfile("tests.txt"):
            ...
            if settings:
                ...
                self.add_logs_system(
                    "Файл {} присутствует но пустой либо неправильного формата".format(
                        "tests.txt"
                    )
                )
                return False
            ...
            self.add_logs_system(
                "Нет файла {} с настройками тестирования".format("tests.txt")
            )
            self.add_logs_system(
                "Для всех циклов будут использоваться настроки поумолчанию"
            )
            return False
    ...
    def gui_initialization_toolbar(self):
        ...
        self.tb_btn_stop_all_process_action = self.toolbar.addWidget(
            self.tb_btn_stop_all_process
        )
    ...
    def gui_start_process(self):
        ...
        self.add_logs_system(
            message="Запущен процесс обработки сайта {}".format(
    ...
    def show_window(self):
        ...
        task_bar_height = (
            desctop.screenGeometry().height() - desctop.availableGeometry().height()
        )
    ...
    def on_timer_cpu_calculate_value(self):
        ...
        self.cpu_current_values = list()
        self.l_kpi_cpu.setText(
            "{} {}".format(self.cpu_current_value, self.cpu_current_trend)
        )
        ...
        if self.cpu_current_value < self.sb_cpu_max.value():
            ...
            if self.process_started:
                self.add_logs_system(
                    message="Идет обработка", status_bar=True, logs_system=False
                )
            else:
                self.add_logs_system(
    ...
    def gui_event_show_stat_in_new_window(self):
        """ """
        win = window_stat(parent=self, table_stat_model=self.table_stat_model)
        win.show()
    ...
    def gui_event_show_requests_logs_in_new_window(self):
        """ """
        win = window_logs(parent=self, table_model=self.table_log_requests_model)
        win.show()
    ...
    def gui_event_requests_settings(self):
        """ """
        win = window_request_settings(parent=self, app_settings=self.app_settings)
        win.show()
    ...
    def graphic_main(self):
        ...
        self.graphics_view_main.plot(
            y=self.graphic_started_theaders_data, pen=pg.mkPen((0, 85, 255), width=2)
        )

        # Active Theaders
        self.graphic_active_theaders_data.append(
            self.count_theaders_by_status(status="running")
        )

        self.graphics_view_main.plot(
            y=self.graphic_active_theaders_data, pen=pg.mkPen((170, 0, 0), width=2)
        )

        # Requests per second
        self.graphic_active_requests_data.append(
            int(self.l_kpi_requests_per_seccond.text())
        )

        self.graphics_view_main.plot(
            y=self.graphic_active_requests_data, pen=pg.mkPen((170, 0, 255), width=2)
        )

        # Average data
        self.label_average_started_theaders.setText(
            "{} / {}".format(
        ...
        self.label_average_active_theaders.setText(
            "{} / {}".format(
        ...
        self.label_average_requests_per_sec.setText(
            "{} / {}".format(
    ...
    def graphic_memory(self):
        ...
        self.graphics_view_memory.plot(
            y=self.graphic_memory_data, pen=pg.mkPen(width=2)
        )
        ...
        midle_value = round(
            sum(self.graphic_memory_data) / len(self.graphic_memory_data)
        )
    ...
    def graphic_cpu(self):
        ...
        self.graphics_view_cpu.plot(y=self.graphic_cpu_data, pen=pg.mkPen(width=2))
        ...
        if cpu_average > self.sb_cpu_max.value():
            self.gb_average_cpu.setStyleSheet(
        ...
        # cpu_current_value
        if self.cpu_current_value > self.sb_cpu_max.value():
            self.gb_cpu_current.setStyleSheet(
                "background-color: rgb(255, 112, 77); color: rgb(255, 255, 255);"
            )
        else:
            self.gb_cpu_current.setStyleSheet(
        ...
        if cpu_average_persent >= 100:
            self.gb_average_cpu_persent.setStyleSheet(
                "background-color: rgb(255, 112, 77); color: rgb(255, 255, 255);"
            )
        else:
            self.gb_average_cpu_persent.setStyleSheet(
    ...
    def graphic_speed(self):
        ...
        self.graphics_view_speed.plot(y=self.graphic_speed_data, pen=pg.mkPen(width=2))
        ...
        midle_value = round(
            sum(self.graphic_speed_data) / len(self.graphic_speed_data), 2
        )
        midle_line.setPos([0, midle_value])
    ...
    def graphic_requests(self):
        ...
        persent_requests_success = round(
            (int(self.l_kpi_requests_success.text()) * 100)
            / int(self.l_kpi_requests.text()),
            2,
        )
        self.graphic_requests_success_data.append(persent_requests_success)

        persent_requests_4xx = round(
            (int(self.l_kpi_requests_4xx.text()) * 100)
            / int(self.l_kpi_requests.text()),
            2,
        )
        ...
        persent_requests_5xx = round(
            (int(self.l_kpi_requests_5xx.text()) * 100)
            / int(self.l_kpi_requests.text()),
            2,
        )
        ...
        persent_requests_connection_err = round(
            (int(self.l_kpi_requests_connection_error.text()) * 100)
            / int(self.l_kpi_requests.text()),
            2,
        )
        self.graphic_requests_connection_err_data.append(
            persent_requests_connection_err
        )

        all_errors = (
            int(self.l_kpi_requests_4xx.text())
        ...
        self.graphics_view_requests.plot(
            y=self.graphic_requests_success_data, pen=pg.mkPen((0, 204, 102), width=2)
        )

        self.graphics_view_requests.plot(
            y=self.graphic_requests_4xx_data, pen=pg.mkPen((255, 102, 102), width=2)
        )

        self.graphics_view_requests.plot(
            y=self.graphic_requests_5xx_data, pen=pg.mkPen((255, 112, 77), width=2)
        )
    ...
    def gui_event_change_request_method(self):
        """ """
        if (
            self.cb_request_method.currentText() == "get"
    ...
    def add_logs_system(self, message, status_bar=False, logs_system=True):
        ...
        if logs_system:
            self.logs_system.appendPlainText(
                "{} \t {}".format(self.cur_date_time(), message.strip())
            )
        if status_bar:
    ...
    def check_url_http_status_code(self, url):
        ...
        try:
            response = requests.head(
                url=url, headers=headers, timeout=self.sb_request_timeout.value()
            )
            if response.status_code in [200, 301, 302]:
                return True
            else:
    ...
    def check_url(self):
        ...
        if self.cb_urls.currentText() and not self.check_url_http_status_code(
    ...
    def on_theader_iteration(self, theader_id, iteration, cpu_value, theader_status):
        ...
        if pg:
            if theader_status:
                self.theaders_objs["theader_" + str(theader_id)]["theader_status"] = (
                    "stoped"
                )
            ...
            pg.setFormat(
                " Поток #{}. Выполнено запросов %v из %m ({} (CPU {}% / {}%))".format(
                    theader_id, theader_status, cpu_value, round(self.cpu_current_value)
                )
            )
        ...
        # Если PG не существует
            ...
            pg.setFormat(
                " Поток #{}. Выполнено запросов %v из %m (running (CPU {}% / {}%))".format(
                    theader_id, cpu_value, round(self.cpu_current_value)
                )
            )
            pg.setStyleSheet(
                """
    ...
    def gui_kpi_active_theaders_avg_update(self):
        """ """
        self.l_kpi_theaders_active_avg.setText(
            str(
                round(
                    sum(self.graphic_active_theaders_data)
                    / len(self.graphic_active_theaders_data)
    ...
    def gui_kpi_started_theaders_avg_update(self):
        """ """
        self.l_kpi_theaders_started_avg.setText(
            str(
                round(
                    sum(self.graphic_started_theaders_data)
                    / len(self.graphic_started_theaders_data)
    ...
    def gui_kpi_requests_success_update(self):
        ...
        self.l_requests_success.setText(
            "{} / {}%".format(self.count_requests_success, requests_success_persent)
        )
    ...
    def gui_kpi_requests_errors_update(self):
        ...
        self.l_requests_errors.setText(
            "{} / {}%".format(requests_errors, requests_errors_persent)
        )
    ...
    def gui_kpi_calculate_speed_update(self):
        ...
        if (
            ...
            self.l_kpi_speed.setText(
                "{}".format(
                    round(
                        (self.download_size / self.get_process_time_in_seconds())
                        / 1048576,
                        2,
                    )
                )
        ...
        if self.cb_request_method.currentText() == "head":
            ...
            self.l_kpi_speed.setText(
                "{}".format(
                    round(
                        (self.download_size / self.get_process_time_in_seconds()) / 1, 2
                    )
                )
            )
    ...
    def check_tests_settings(self, cycle_count):
        ...
        if self.sb_cycles_count.value() > 1:
            ...
            if self.app_tests:
                self.sb_cpu_max.setValue(
                    self.get_tests_value(
                        cycle_count, "cpu_limit", self.sb_cpu_max.value()
                    )
                )
                self.sb_theaders.setValue(
                    self.get_tests_value(
                        cycle_count, "theaders", self.sb_theaders.value()
                    )
                )
                self.sb_requests.setValue(
                    self.get_tests_value(
                        cycle_count, "requests", self.sb_requests.value()
                    )
                )
            ...
            for cycle_count in range(0, self.sb_cycles_count.value()):
                ...
                while theader_id <= self.sb_theaders.value():
                    ...
                    try:
                        ...
                        theader.signals.on_log_message.connect(
                            self.on_theader_log_message
                        )
                        theader.signals.on_iteration.connect(self.on_theader_iteration)
                        theader.signals.on_theader_finished.connect(
                            self.on_theader_finished
                        )
                        theader.signals.on_theader_started.connect(
                ...
                while True:
                    if self.count_theaders_by_status(status="finished") == len(
                        self.theaders_objs
                    ):
                        break
                    self.set_theaders_system_cpu()
