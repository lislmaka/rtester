...
class grafs():
    ...
    def graphic_main(self):
        ...
        # Active Theaders
        self.parent.graphic_active_theaders_data.append(self.parent.count_theaders_by_status(status="running"))
        ...
        # Requests per second
        self.parent.graphic_active_requests_data.append(int(self.parent.l_kpi_requests_per_seccond.text()))
        ...
        # Average data
        self.parent.label_average_started_theaders.setText("{} / {}".format(
            self.parent.graphic_started_theaders_data[-1],
            round(sum(self.parent.graphic_started_theaders_data) / len(self.parent.graphic_started_theaders_data))))

        self.parent.label_average_active_theaders.setText("{} / {}".format(
            self.parent.graphic_active_theaders_data[-1],
            round(sum(self.parent.graphic_active_theaders_data) / len(self.parent.graphic_active_theaders_data))))

        self.parent.label_average_requests_per_sec.setText("{} / {}".format(
            self.parent.graphic_active_requests_data[-1],
            round(sum(self.parent.graphic_active_requests_data) / len(self.parent.graphic_active_requests_data))))
    ...
    def graphic_memory(self):
        ...
        self.parent.graphics_view_memory.plot(
            y=self.parent.graphic_memory_data,
    ...
    def graphic_cpu(self):
        ...
        self.parent.graphics_view_cpu.plot(
            y=self.parent.graphic_cpu_data,
            pen=pg.mkPen(width=2))
        ...
        midle_value = round(sum(self.parent.graphic_cpu_data) / len(self.parent.graphic_cpu_data), 2)
        midle_line.setPos([0, midle_value])
        ...
        #
        cpu_average = round(sum(self.parent.graphic_cpu_data) / len(self.parent.graphic_cpu_data), 2)
        cpu_average_persent = round(cpu_average * 100 / self.parent.sb_cpu_max.value(), 2)
        ...
        if cpu_average > self.parent.sb_cpu_max.value():
            self.parent.gb_average_cpu.setStyleSheet("background-color: rgb(255, 112, 77); color: rgb(255, 255, 255);")
        else:
            self.parent.gb_average_cpu.setStyleSheet("background-color: rgb(0, 204, 102); color: rgb(255, 255, 255);")
        # cpu_current_value
        if self.parent.cpu_current_value > self.parent.sb_cpu_max.value():
            self.parent.gb_cpu_current.setStyleSheet("background-color: rgb(255, 112, 77); color: rgb(255, 255, 255);")
        else:
            self.parent.gb_cpu_current.setStyleSheet("background-color: rgb(0, 204, 102); color: rgb(255, 255, 255);")
        # cpu_average_persent
        if cpu_average_persent >= 100:
            self.parent.gb_average_cpu_persent.setStyleSheet("background-color: rgb(255, 112, 77); color: rgb(255, 255, 255);")
        else:
            self.parent.gb_average_cpu_persent.setStyleSheet("background-color: rgb(0, 204, 102); color: rgb(255, 255, 255);")
    ...
    def graphic_speed(self):
        ...
        self.parent.graphics_view_speed.plot(
            y=self.parent.graphic_speed_data,
            pen=pg.mkPen(width=2))
    ...
    def graphic_requests(self):
        """
        """
        persent_requests_success = round((int(self.parent.l_kpi_requests_success.text()) * 100)
                                         / (self.parent.sb_requests_per_theader.value() * self.parent.sb_theaders.value()), 2)
        self.parent.graphic_requests_success_data.append(persent_requests_success)

        persent_requests_4xx = round((int(self.parent.l_kpi_requests_4xx.text()) * 100)
                                     / (self.parent.sb_requests_per_theader.value() * self.parent.sb_theaders.value()), 2)
        self.parent.graphic_requests_4xx_data.append(persent_requests_4xx)

        persent_requests_5xx = round((int(self.parent.l_kpi_requests_5xx.text()) * 100)
                                     / (self.parent.sb_requests_per_theader.value() * self.parent.sb_theaders.value()), 2)
        self.parent.graphic_requests_5xx_data.append(persent_requests_5xx)

        persent_requests_connection_err = round((int(self.parent.l_kpi_requests_connection_error.text()) * 100)
                                                / (self.parent.sb_requests_per_theader.value() * self.parent.sb_theaders.value()), 2)
        self.parent.graphic_requests_connection_err_data.append(persent_requests_connection_err)

        all_errors = (int(self.parent.l_kpi_requests_4xx.text()) +
